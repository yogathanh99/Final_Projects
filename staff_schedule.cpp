...
void ImportScheduleFromFile(const vector<Course> &courses, vector<Schedule> &schedules) {
	...
	while (!fin.eof()) {
		...
		if (result) {
			cout << "Warning: the following schedule" << endl;
			printSchedule(schedule);
			cout << "conflicted with the following schedule" << endl;
			printSchedule(conflictedSchedule);
			cout << "Import anyway (Y/N)? ";
			string respond;
			getline(cin, respond);
			if (respond == "N")
				continue;
...
void AddASchedule(const vector<Course> &courses, vector<Schedule> &schedules) {
	...
	string s = findLecturer(courses, schedule);
	if (s == "N/A") {
		cout << "Can not find course with given course code, year and semester.";
		Pause();
		return;
	}
